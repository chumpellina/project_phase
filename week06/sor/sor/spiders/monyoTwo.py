...
class SorSpider (CrawlSpider):
    ...
    def parseTwo(self, response):
        ...
        raw_ingredients = sel.xpath("//div [@class='textwidget'][2]/text()").extract()
        item["ingredients"] = "".join(raw_ingredients)

        raw_temperature = sel.xpath("//div [@class='textwidget'][3]/text()").extract()
        item["temperature"] = "".join(raw_temperature)

        raw_alcohol_vol = sel.xpath("//div [@class='textwidget'][4]/text()").extract()
        item["alcohol_vol"] = "".join(raw_alcohol_vol)

        raw_bitterness = sel.xpath("//div [@class='textwidget'][4]/text()").extract()
        item["bitterness"] = "".join(raw_bitterness)

        raw_color = sel.xpath("//div [@class='textwidget'][4]/text()").extract()
